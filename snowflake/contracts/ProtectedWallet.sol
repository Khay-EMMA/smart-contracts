...
contract ProtectedWallet is SnowflakeResolver, Chainlinked {
    ...
    uint256 private pendingDailyLimit;
    uint256 private oneTimeWithdrawalAmount;
    uint256 private oneTimeTransferExtAmount;
    address private oneTimeTransferExtAddress;
    uint256 private oneTimeWithdrawalExtAmount;
    uint256 private oneTimeWithdrawalExtEin;

    uint256 private hydroBalance;
    bool private hasPassword;
    bool private resolverAdded;
    uint256 private withdrawnToday;
    ...
    event DepositFromSnowflake(
        uint256 indexed _ein,
        uint256 indexed _amount,
        address _from
    );
    event DepositFromAddress(uint256 indexed _amount, address indexed _from);
    event WithdrawToSnowflake(uint256 indexed _ein, uint256 indexed _amount);
    event WithdrawToAddress(address indexed _to, uint256 indexed _amount);
    event RaindropMessage(uint256 indexed shortMessage);
    ...
    bytes32 ONETIME_TRANSFEREXT_JOB = bytes32(
        "f26b8184a106449c9a81ccdae1ef0b5c"
    );
    bytes32 ONETIME_WITHDRAWEXT_JOB = bytes32(
        "f26b8184a106449c9a81ccdae1ef0b5c"
    );
    ...
    constructor(
        ...
        address clientRaindropAddr
    )
        public
        SnowflakeResolver(
            "Your personal protected wallet",
            "Protect your funds without locking them up in cold storage",
            snowflakeAddress,
            true,
            true
        )
    ...
    modifier walletHasPassword() {
        require(hasPassword == true, "Wallet must have password");
        _;
    }
    ...
    function getLinkBalance() public returns (uint256) {
    ...
    function getEIN() public view returns (uint256) {
        return ein;
    }
    ...
    function getWithdrawnToday() public view returns (uint256) {
    ...
    function setJobIds(bytes16[5] memory ids) public {
        require(
            idRegistry.getEIN(msg.sender) == ein,
            "Only the ein that owns this wallet can make withdrawals"
        );
    ...
    function receiveApproval(
        ...
        require(
            _tokenAddress == address(hydro),
            "Token address is not the HYDRO token contract"
        );
        depositFromAddress(sender, value);
    }

    function depositFromAddress(address sender, uint256 value) internal {
        require(
            hydro.transferFrom(sender, address(this), value),
            "Must transfer token from sender"
        );
        hydroBalance = hydroBalance.add(value);
        emit DepositFromAddress(value, address(this));
    }

    function depositFromSnowflake(uint256 amount) public {
        uint256 _ein = idRegistry.getEIN(msg.sender);
        hydroBalance = hydroBalance.add(amount);
        snowflake.withdrawSnowflakeBalanceFrom(_ein, address(this), amount);
        emit DepositFromSnowflake(
            idRegistry.getEIN(msg.sender),
            amount,
            msg.sender
        );
    }

    function withdrawToAddress(uint256 amount, address addr) public {
        require(
            amount <= dailyLimit,
            "Can only make withdrawals up to daily limit"
        );
    ...
    function withdrawToSnowflake(uint256 amount, uint256 _einTo) public {
        require(
            idRegistry.getEIN(msg.sender) == ein,
            "Only the ein that owns this wallet can make withdrawals"
        );
    ...
    function requestChangeDailyLimit(uint256 newDailyLimit) public {
        require(
            idRegistry.getEIN(msg.sender) == ein,
            "Only the ein that owns this wallet can make withdrawals"
        );
        require(
            pendingDailyLimit == 0,
            "A change daily limit request is already in progress"
        );
        ChainlinkLib.Run memory run = newRun(
            LIMIT_JOB,
            address(this),
            this.fulfillChangeDailyLimit.selector
        );
        run.add("role", "client");
        run.add("hydroid", hydroId);
        uint256 longMessage = uint256(blockhash(block.number - 1));
        uint256 shortMessage = longMessage % 1000000;
    ...
    function requestChainlinkRecover() public {
        require(
            idRegistry.getEIN(msg.sender) == ein,
            "Only addresses associated with this wallet ein can invoke this function"
        );
        require(
            pendingRecovery == false,
            "Recovery request already in progress"
        );
        ChainlinkLib.Run memory run = newRun(
            RECOVER_JOB,
            address(this),
            this.fulfillChainlinkRecover.selector
        );
        run.add("role", "client");
        run.add("hydroid", hydroId);
        uint256 longMessage = uint256(blockhash(block.number - 1));
    ...
    function requestOneTimeTransferExternal(
        uint256 amount,
        address _to,
        uint256 numHydro,
        ...
        require(
            ...
            "A transfer request is already in progress"
        );
        require(
            oneTimeTransferExtAddress == address(0),
            "Transfer address must be reset"
        );
        oneTimeTransferExtAmount = amount;
        oneTimeTransferExtAddress = _to;
        ChainlinkLib.Run memory run = newRun(
            ONETIME_TRANSFEREXT_JOB,
            address(this),
            this.fulfillOneTimeTransferExternal.selector
        );
        run.add("role", "client");
        run.add("hydroid", hydroId);
        uint256 longMessage = uint256(blockhash(block.number - 1));
    ...
    function requestOneTimeWithdrawalExternal(uint256 amount, uint256 einTo)
        public
    {
        require(
            idRegistry.getEIN(msg.sender) == ein,
        ...
        require(
            ...
            "Withdrawal to external ein already initiated"
        );
        require(
        ...
        ChainlinkLib.Run memory run = newRun(
            ONETIME_WITHDRAWEXT_JOB,
            address(this),
            this.fulfillOneTimeWithdrawalExternal.selector
        );
        run.add("role", "client");
        run.add("hydroid", hydroId);
        uint256 longMessage = uint256(blockhash(block.number - 1));
        ...
        returns (bool)
    {
        if (_response == true) {
            withdrawHydroBalanceTo(
                oneTimeTransferExtAddress,
                oneTimeTransferExtAmount
            );
    ...
    function fulfillOneTimeWithdrawalExternal(
        bytes32 _requestId,
        bool _response
    ) public checkChainlinkFulfillment(_requestId) returns (bool) {
        if (_response == true) {
            transferHydroBalanceTo(
                oneTimeWithdrawalExtEin,
                oneTimeWithdrawalExtAmount
            );
    ...
    function revealAndRecover(
        bytes32 _hash,
        address payable _dest,
        string memory password
    ) public {
        require(
            keccak256(abi.encodePacked(address(uint160(_dest)), password)) ==
                _hash,
            "Hashed input values not equal to commit hash"
        );
        bytes32 passHash = keccak256(abi.encodePacked(password));
        require(
            keccak256(abi.encodePacked(address(this), passHash)) == oneTimePass,
            "Invalid password"
        );
    ...
    function onAddition(
        ...
        ChainlinkLib.Run memory run = newRun(
            HYDRO_ID_JOB,
            address(this),
            this.fulfillOnAddition.selector
        );
    ...
    function onRemoval(uint256 ein, bytes memory extraData)
        public
        senderIsSnowflake()
        returns (bool)
    {
